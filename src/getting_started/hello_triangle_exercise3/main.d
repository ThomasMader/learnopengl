...
string vertexShaderSource = 
    ...
    "}";
string orangeFragmentShaderSource =
    "#version 330 core\n" ~
    "out vec4 FragColor;\n" ~
    "void main()\n" ~
    "{\n" ~
    ...
    "}";
string yellowFragmentShaderSource =
    "#version 330 core\n" ~
    "out vec4 FragColor;\n" ~
    "void main()\n" ~
    "{\n" ~
...
void main()
{
    ...
    GLchar* fragmentShaderSourcePtr = cast(GLchar*)(toStringz(orangeFragmentShaderSource));
    glShaderSource(orangeFragmentShader, 1, &fragmentShaderSourcePtr, null);
    glCompileShader(orangeFragmentShader);
    // check for shader compile errors
    glGetShaderiv(orangeFragmentShader, GL_COMPILE_STATUS, &success);
    if (!success)
    {
        glGetShaderInfoLog(orangeFragmentShader, 512, null, infoLog.ptr);
        writeln("ERROR::SHADER::FRAGMENT::ORANGE::COMPILATION_FAILED", infoLog);
    }
    // link shaders
    ...
    if (!success) {
        glGetProgramInfoLog(orangeShaderProgram, 512, null, infoLog.ptr);
    ...
    fragmentShaderSourcePtr = cast(GLchar*)(toStringz(yellowFragmentShaderSource));
    glShaderSource(yellowFragmentShader, 1, &fragmentShaderSourcePtr, null);
    glCompileShader(yellowFragmentShader);
    // check for shader compile errors
    glGetShaderiv(yellowFragmentShader, GL_COMPILE_STATUS, &success);
    if (!success)
    {
        glGetShaderInfoLog(yellowFragmentShader, 512, null, infoLog.ptr);
        writeln("ERROR::SHADER::FRAGMENT::YELLOW::COMPILATION_FAILED", infoLog);
    }
    // link shaders
    ...
    if (!success) {
        glGetProgramInfoLog(yellowShaderProgram, 512, null, infoLog.ptr);
        writeln("ERROR::SHADER::PROGRAM::LINKING_FAILED", infoLog);
    }
